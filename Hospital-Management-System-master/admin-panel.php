    ...
    if((date("Y-m-d",$appdate1)==$cur_date and date("H:i:s",$apptime1)>$cur_time) or date("Y-m-d",$appdate1)>$cur_date) {
      $check_query = mysqli_query($con,"select apptime from appointmenttb where doctor='$doctor' and appdate='$appdate' and apptime='$apptime'");
        ...
        if(mysqli_num_rows($check_query)==0){
          ...
          // --- BEGIN CORRECTION ---
          // 1. Define the query string FIRST.
          $query_string = "insert into appointmenttb(pid,fname,lname,gender,email,contact,doctor,docFees,appdate,apptime,userStatus,doctorStatus)
                           values('$pid','$fname','$lname','$gender','$email','$contact','$doctor','$docFees','$appdate','$apptime','1','1')";

          // 2. Execute the query using the defined string.
          ...
          if($query_result)
          {
            ...
            // IMPORTANT: If you still get a SQL error, this line will help you find it!
            echo "<script>alert('Unable to process your request. SQL Error: ".mysqli_error($con)."');</script>";
          }
          // --- END CORRECTION ---
      }
      else{
